...
def modify_motion(frames,alpha,D,N,K,F,verbose=True, monitor=True):
    '''
    ...
    frames, rows, cols = Ps[0][0][0].shape

    td = 0
    tn = 0

    if monitor:
        monitorVals = np.zeros((rows*2, cols*4, frames+pad))
    else:
        monitorVals = None
    ...
    for d in range(D):
        for n in range(N):
            for k in range(K):
                ...
                if d == td and n == tn and isinstance(monitorVals, np.ndarray):
                    kcol = k % 4
                    krow = k // 4
                    monitorVals[rows*krow : rows*(krow+1), cols*kcol : cols*(kcol+1), : T] = np.abs(P_frames)
    ...
    if isinstance(monitorVals, np.ndarray):
        return mframes, np.moveaxis(monitorVals,-1,0)
    else:
        return mframes, None
